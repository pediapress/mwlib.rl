    ...
    def removeBadArticles(self, book, output, removedArticlesFile):
        ...
        for (i,node) in enumerate(book.parseTree):
            if isinstance(node, Article):
                ...
                try:
                    testdoc = BaseDocTemplate(output, topMargin=pageMarginVert, leftMargin=pageMarginHor, rightMargin=pageMarginHor, bottomMargin=pageMarginVert, title=book.title)
